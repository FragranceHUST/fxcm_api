...
| 批复 0910 | 回测主区间=**过去 5 年**；数据完整验证通过前不启动回测开发（M0 门槛） |
...
| B10 | 回测主区间：按批复定为**过去 5 年**（2021-09→今）；XAU m1 当前深度 ~3.2 年且仍在续挖，若最终不足 5 年则 5 年内可用段为主区间 + 10 年 H4 粗粒度对照 | 双档并行（主结论用 m1 档，长周期用 H4 档做稳健性对照） |
